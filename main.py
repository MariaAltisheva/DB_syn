...
def display_item(name):
    """Выводит на экран все позиции с совпадабщими названиями позиций"""
    conn = None
    results = []
    try:
        conn = sqlite3.connect('inventory.db')
...
def update_row(id, name, price):
    ...
    try:
        conn = sqlite3.connect('inventory.db')
        cur = conn.cursor()
        cur.execute('''UPDATE Inventory SET ItemName = ?, Price = ? WHERE ID == ?''', (id, name, price))
        conn.commit()
...
def main():
    ...
    choice = 0
    while choice != EXIT:
        display_menu() # обращаемся к функции display menu, с помощью которой на экран выводится меню
